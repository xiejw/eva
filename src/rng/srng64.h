...
// Cast srng64_t* to rng64_t* for use.
struct rng64_t {
        uint64_t seed_;
        uint64_t gamma_;
};
...
extern uint64_t rng64NextUint64(struct rng64_t* prng);
extern uint32_t rng64NextUint32(struct rng64_t* prng);
extern double   rng64NextDouble(struct rng64_t* prng);
